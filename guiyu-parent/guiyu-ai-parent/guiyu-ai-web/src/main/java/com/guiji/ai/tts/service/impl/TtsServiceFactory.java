...
public class TtsServiceFactory {
	...
	public static ITtsServiceProvide getTtsProvide(String model) {
		...
		try {
			...
			do{
				avaliableGpuList = (List<GuiyuTtsGpu>) redisUtil.get(TtsConstants.GUIYUTTS + model + TtsConstants.AVALIABLE);
			...
			if (!avaliableGpuList.isEmpty()) { // 取到可用GPU
				...
				unavaliableGpuList = (List<GuiyuTtsGpu>) redisUtil.get(TtsConstants.GUIYUTTS + model + TtsConstants.UNAVALIABLE);
