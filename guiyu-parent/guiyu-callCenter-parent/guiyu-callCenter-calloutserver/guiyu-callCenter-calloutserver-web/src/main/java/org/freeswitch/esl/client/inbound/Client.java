...
public class Client
{
     ...
     * response from the server provides a UUID to identify the eventbus execution results. When the server
     * has completed the eventbus execution it fires a BACKGROUND_JOB Event with the execution results.<p/>
    ...
    /**
     * 将申请的资源全部释放掉，否则会影响后续的重连
     */
    public void destroy(){
        try {
            channel.disconnect().awaitUninterruptibly();
        }catch (Exception ex){
            log.warn("destroy: channel.disconnect出现异常");
        }

        try {
            channel.close().awaitUninterruptibly();
        }catch (Exception ex){
            log.warn("destroy: channel.close出现异常");
        }
        ...
        try{
            ...
            log.warn("destroy：bootstrap在释放资源时出现异常", ex);
        }
    }
    ...
    private final IEslProtocolListener protocolListener = new IEslProtocolListener()
    {
        ...
        public void eventReceived( final EslEvent event )
        {
             ...
             *  Use a different worker thread pool for async eventbus results than for event driven
                ...
                for ( final IEslEventListener listener : eventListeners )
                {
                    backgroundJobListenerExecutor.execute( new Runnable()
                    {
                        public void run()
                        {
                            ...
                            {
                                log.error( "Error caught notifying listener of eventbus result [" + event + ']', t );
                            }
                        }
                    } );
                }
