...
public interface INoticeLabel {
    ...
    Result.ReturnData<String> queryNoticeIntent(@NotEmpty(message = "参数不能为空") String orgCode);

    @ApiOperation(value = "修改意向标签,勾选的标签以逗号拼接传递到labels字段")
    @GetMapping(value = "updateNoticeIntent")
    Result.ReturnData updateNoticeIntent(@NotEmpty(message = "参数不能为空") String orgCode, @NotNull(message = "标签不能不传") String labels);
}
