...
public class GroupUtils {
	...
	* @ClassName: GroupBy 
	...
	public static <K, V> void listGroup2Map(List<V> list, Map<K, List<V>> map, Class<V> clazz, String methodName) {
		// 入参非法行校验
		if (null == list || null == map || null == clazz || methodName == null || methodName.equals("")) {
	...
	public static Method getMethodByName(Class<?> clazz, String methodName) {
		Method method = null;
		// 入参不能为空
		if (null == clazz || methodName == null || methodName.equals("")) {
	...
	public static void main(String[] args) {
		/**
		 * 有两种分组方式，性能经测试，差别不是很大，都可以使用
		 */
//		Map<Long, List<Data>> result1Map = new LinkedHashMap<Long, List<Data>>();
//		GroupUtils.listGroup2Map(list, result1Map, Data.class, "getId");// 输入方法名(根据该方法返回值分组)
		
		//方法二（也是根据方法的某个返回值分组）
//		Map<Long, List<Data>> map = GroupUtils.group(list, new GroupBy<Long>() {
//			@Override
//			public Long groupby(Object obj) {
//				Data d = (Data) obj;
//				return d.getId(); // 分组依据为课程ID
//			}
//		});
	}
}
