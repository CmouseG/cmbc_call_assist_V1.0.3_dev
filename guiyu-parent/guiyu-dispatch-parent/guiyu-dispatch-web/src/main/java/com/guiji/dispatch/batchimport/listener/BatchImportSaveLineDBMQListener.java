...
@Component
@RabbitListener(queues = "fanout.dispatch.BatchImportSaveLineDB.saveDb", containerFactory = "batchImportSaveLineDBRabbitFactory")
public class BatchImportSaveLineDBMQListener
{
	...
	public void process(String message, Channel channel, Message message2) {
		...
		DispatchLines vo = JsonUtils.json2Bean(message, DispatchLines.class);
		//executorService.execute(new BatchImportThread(vo, handler));
		try
		{
			lineService.insertLines(vo);
