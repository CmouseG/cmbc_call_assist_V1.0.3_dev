...
CREATE TABLE `sms_tunnel` (
  ...
  `platform_name` varchar(32) DEFAULT NULL COMMENT '平台名称',
  `tunnel_name` varchar(32) DEFAULT NULL COMMENT '通道名称',
...
CREATE TABLE `sms_config` (
  ...
  `template_name` varchar(64) DEFAULT NULL COMMENT '话术模版名称',
...
CREATE TABLE `sms_task` (
  ...
  `send_type` int(1) DEFAULT NULL COMMENT '发送方式：0-定时发送；1-手动发送',
  `tunnel_name` varchar(32) DEFAULT NULL COMMENT '通道名称',
  `sms_content` text COMMENT '自定义短信内容',
...
DROP TABLE IF EXISTS `sms_task_detail`;
CREATE TABLE `sms_task_detail` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `planuuid` varchar(32) DEFAULT NULL COMMENT '挂机短信唯一标识',
  `task_name` varchar(128) DEFAULT NULL COMMENT '任务名称',
  `phone` varchar(11) DEFAULT NULL COMMENT '客户电话',
  `sms_content` text COMMENT '短信内容',
  `send_type` int(1) DEFAULT NULL COMMENT '发送方式：0-手动发送；1-定时发送；2-调用发送',
